...
async def analyze_images(images_base64: List[str]) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"[analyze_images] Sending {num_pages} image(s) to Claude Vision API ({VISION_MODEL})")
        message = await client.messages.create(
            model=VISION_MODEL,
            max_tokens=4096,
            system=(
...
async def generate_questions(
    ...
    try:
        logger.info(f"[generate_questions] type={test_type}, model={TEXT_MODEL}, num={num_questions}, content_len={len(content_text)}")
        message = await client.messages.create(
            model=TEXT_MODEL,
            max_tokens=4096,
            system=system_msg,
...
async def generate_tutor_response(
    ...
    # Use vision model when an image is attached (better OCR on student homework),
    # text model otherwise (faster/cheaper for plain Q&A).
    tutor_model = VISION_MODEL if image_base64 else TEXT_MODEL
